...
void Preprocess::mid360_handler(const sensor_msgs::msg::PointCloud2::UniquePtr &msg)
{
  ...
  for (uint i = 0; i < plsize; ++i)
  {
    ...
    added_pt.curvature = given_offset_time ? static_cast<float>(pl_orig.points[i].time * time_unit_scale) : 0.;
    ...
    if (!given_offset_time)
    {
      // compute offset time from yaw ordering when per-point timing is unavailable
      if (yaw_angle <= yaw_fp[layer])
      {
        added_pt.curvature = (yaw_fp[layer] - yaw_angle) / omega_l;
      }
