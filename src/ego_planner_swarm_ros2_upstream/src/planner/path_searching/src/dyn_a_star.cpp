...
{
    constexpr double kAstarTimeLimitSec = 0.5;
    ...
    while (!openSet_.empty())
    {
        ...
        if ((time_2 - time_1).seconds() > kAstarTimeLimitSec)
        {
            RCLCPP_WARN(rclcpp::get_logger("AstarSearch"), "Failed in A star path searching !!! %.1f seconds time limit exceeded.", kAstarTimeLimitSec);
            return false;
        }
    }
