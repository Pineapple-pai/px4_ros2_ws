...
namespace ego_planner
{
  ...
  namespace
  {
    bool clampPointToMapInterior(const GridMap::Ptr &grid_map, Eigen::Vector3d &pt)
    {
      if (!grid_map)
      {
        return false;
      ...
      const double margin = std::max(resolution, 1e-3);
      const Eigen::Vector3d lower = origin + Eigen::Vector3d::Constant(margin);
      const Eigen::Vector3d upper = origin + size - Eigen::Vector3d::Constant(margin);

      for (int axis = 0; axis < 3; ++axis)
      {
        pt(axis) = std::min(std::max(pt(axis), lower(axis)), upper(axis));
      }

      return grid_map->isInMap(pt);
    }

    bool isKnownFreePoint(const GridMap::Ptr &grid_map, const Eigen::Vector3d &pt)
    {
      if (!grid_map || !grid_map->isInMap(pt))
      {
        return false;
      }

      Eigen::Vector3i idx;
      grid_map->posToIndex(pt, idx);
      return grid_map->isKnownFree(idx);
    }

    bool findNearestCollisionFreePoint(const GridMap::Ptr &grid_map,
                                       const Eigen::Vector3d &seed,
                                       Eigen::Vector3d &adjusted_pt,
                                       bool prefer_known_free)
    {
      if (!grid_map)
      {
        return false;
      ...
      Eigen::Vector3d search_seed = seed;
      if (!clampPointToMapInterior(grid_map, search_seed))
      {
        return false;
      }

      if (grid_map->getInflateOccupancy(search_seed) == 0 &&
          (!prefer_known_free || isKnownFreePoint(grid_map, search_seed)))
      {
        adjusted_pt = search_seed;
        return true;
      }

      constexpr int kMaxSearchCells = 12;
      bool found_fallback = false;
      double fallback_dist = std::numeric_limits<double>::max();
      Eigen::Vector3d fallback_pt = search_seed;

      Eigen::Vector3i center_idx;
      grid_map->posToIndex(search_seed, center_idx);

      for (int radius = 1; radius <= kMaxSearchCells; ++radius)
      {
        for (int dx = -radius; dx <= radius; ++dx)
        {
          for (int dy = -radius; dy <= radius; ++dy)
          {
            for (int dz = -radius; dz <= radius; ++dz)
            {
              ...
              if (!grid_map->isInMap(idx))
              {
                continue;
              }

              Eigen::Vector3d candidate;
              grid_map->indexToPos(idx, candidate);

              if (grid_map->getInflateOccupancy(candidate) != 0)
              {
                continue;
              }

              if (prefer_known_free && isKnownFreePoint(grid_map, candidate))
              {
                adjusted_pt = candidate;
                return true;
              }

              const double dist = (candidate - search_seed).squaredNorm();
              if (!found_fallback || dist < fallback_dist)
              {
                found_fallback = true;
                fallback_dist = dist;
                fallback_pt = candidate;
              }
            }
          }
        }
    ...
    bool sanitizePlanningPoint(const GridMap::Ptr &grid_map,
                               const char *label,
                               const Eigen::Vector3d &original_pt,
                               Eigen::Vector3d &pt,
                               bool prefer_known_free)
    {
      if (!grid_map)
      ...
      Eigen::Vector3d candidate = original_pt;
      const bool was_in_map = grid_map->isInMap(candidate);
      if (!clampPointToMapInterior(grid_map, candidate))
      {
        RCLCPP_ERROR(rclcpp::get_logger("ego_planner"),
                     "%s point cannot be clamped into map bounds: [%.3f, %.3f, %.3f]",
                     label, original_pt(0), original_pt(1), original_pt(2));
        return false;
      }

      if (!was_in_map || grid_map->getInflateOccupancy(candidate) != 0 ||
          (prefer_known_free && !isKnownFreePoint(grid_map, candidate)))
      {
        Eigen::Vector3d adjusted = candidate;
        if (!findNearestCollisionFreePoint(grid_map, candidate, adjusted, prefer_known_free))
        {
          RCLCPP_ERROR(rclcpp::get_logger("ego_planner"),
                       "Unable to find a collision-free %s point near [%.3f, %.3f, %.3f]",
                       label, candidate(0), candidate(1), candidate(2));
          return false;
        }
      ...
      if ((candidate - original_pt).norm() > 1e-3)
      {
        RCLCPP_WARN(rclcpp::get_logger("ego_planner"),
                    "Adjusted %s point from [%.3f, %.3f, %.3f] to [%.3f, %.3f, %.3f]",
                    label,
                    original_pt(0), original_pt(1), original_pt(2),
  ...
  void EGOPlannerManager::initPlanModules(rclcpp::Node::SharedPtr &node, PlanningVisualization::Ptr vis)
  {
    ...
    node->declare_parameter("manager/astar_pool_size_x", 180);
    node->declare_parameter("manager/astar_pool_size_y", 180);
    node->declare_parameter("manager/astar_pool_size_z", 120);
    ...
    int astar_pool_size_x = 180;
    int astar_pool_size_y = 180;
    int astar_pool_size_z = 120;
    ...
    bspline_optimizer_->a_star_->initGridMap(
        grid_map_,
        Eigen::Vector3i(astar_pool_size_x, astar_pool_size_y, astar_pool_size_z));
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    if (!sanitizePlanningPoint(grid_map_, "start", start_pt, start_pt, true))
    {
      continous_failures_count_++;
      return false;
    }

    if (!sanitizePlanningPoint(grid_map_, "local target", local_target_pt, local_target_pt, false))
    {
      continous_failures_count_++;
      return false;
    }
