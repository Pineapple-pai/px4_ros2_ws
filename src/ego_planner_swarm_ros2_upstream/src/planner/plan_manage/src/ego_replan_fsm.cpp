...
namespace ego_planner
{
  ...
  void EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp)
  {
    ...
    if (success)
    {
      ...
      const double global_duration = planner_manager_->global_data_.global_duration_;
      if (!std::isfinite(global_duration) || global_duration <= 0.0)
      {
        RCLCPP_ERROR(node_->get_logger(),
                     "Invalid global trajectory duration: %.6f",
                     global_duration);
        return;
      }

      constexpr int max_visualization_samples = 20000;
      int i_end = std::min(max_visualization_samples,
                           std::max(1, static_cast<int>(std::floor(global_duration / step_size_t))));
  ...
  void EGOReplanFSM::waypointCallback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
  {
    ...
    Eigen::Vector3d requested_goal(
        msg->pose.position.x,
        msg->pose.position.y,
        msg->pose.position.z);

    if (last_manual_goal_.allFinite() && (requested_goal - last_manual_goal_).norm() < 0.05)
      return;

    pending_goal_ = requested_goal;
    last_manual_goal_ = requested_goal;
    pending_manual_goal_ = true;
  }

  void EGOReplanFSM::odometryCallback(const std::shared_ptr<const nav_msgs::msg::Odometry> &msg)
  {
    static bool logged_first_odom = false;
    ...
    if (!logged_first_odom)
    {
      ...
      RCLCPP_INFO(node_->get_logger(),
                  "Received first odom_world message: pos=[%.3f, %.3f, %.3f] vel=[%.3f, %.3f, %.3f]",
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    if (pending_manual_goal_)
    {
      cout << "Triggered!" << endl;
      init_pt_ = odom_pos_;
      pending_manual_goal_ = false;
  ...
  {
    double t = planner_manager_->global_data_.last_progress_time_;

    double t_step = planning_horizen_ / 20 / planner_manager_->pp_.max_vel_;
    double dist_min = 9999, dist_min_t = 0.0;
    bool found_horizon_point = false;
    ...
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      ...
      if (dist >= planning_horizen_)
      {
        ...
        found_horizon_point = true;
        break;
      }
    }
    if (!found_horizon_point) // Last global point or a short-horizon boundary case.
    {
      local_target_pt_ = end_pt_;
    ...
    {
      const double sample_t = std::min(t, planner_manager_->global_data_.global_duration_);
      local_target_vel_ = planner_manager_->global_data_.getVelocity(sample_t);
    }
  }
